...
runInitialProdAdv <- function(analysisId = as.numeric(Sys.time()),
  ...
  for (t in traits_to_use) {
    ...
    arg_values <- c(
      ...
      args$staStamp,
      "Weighted index",
      if (!is.null(args$topPctSelected)) as.character(args$topPctSelected) else NA_character_,
      if (!is.null(args$nSelected)) as.character(args$nSelected) else NA_character_,
